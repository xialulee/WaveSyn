...
class PhysicalQuantitySpinbox(LabeledSpinbox):
    """
    ...
    def __on_unit_change(self, *args):
        ...
        try:
            value_float = float(self.spinbox_value)  # Parse the current value as a float
            new_unit_name = self.__unit_var.get()
            new_unit_obj = self.__unit_info[new_unit_name]
            
            # Rescale the value to the new unit and update the spinbox
            new_value = (value_float * previous_unit_obj).rescale(new_unit_obj).magnitude
            self.__previous_unit_name = new_unit_name
            self.spinbox_value = str(new_value)
        except ValueError:
            # Clear the spinbox if the value is invalid
            self.spinbox_value = ""
...
if __name__ == "__main__":
    ...
    pqent = PhysicalQuantitySpinbox(root, 
        unit_info={
            "km": pq.kilometer, 
            "m":  pq.meter, 
            "cm": pq.centimeter, 
